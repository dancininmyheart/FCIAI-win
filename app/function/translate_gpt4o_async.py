...
async def Translate_texts(field, text, stop_words, custom_translations, source_language, target_language):
    ...
    # GPT API接口URL
    print("调用GPT接口翻译")
    api_url = "http://117.50.216.15/agent_server/app/run/65c9b2a92645477b989328f68faff372"
    ...
    for attempt in range(max_retries + 1):
        try:
            ...
            logger.error(f"GPT翻译API调用失败 (尝试 {attempt + 1}/{max_retries + 1}): {str(e)}")
            if attempt < max_retries:
                ...
                # 所有重试都失败了
                raise Exception(f"GPT翻译API调用失败，已重试{max_retries}次: {str(e)}")

async def translate_gpt4o_async(text: str, field: str = None, stop_words: List[str] = None,
