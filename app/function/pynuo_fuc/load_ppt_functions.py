"""
load_ppt_functions.py (Windows专用版)
PPT加载功能，支持子进程调用
...
def load_entire_ppt_direct(ppt_path, page_indices=None):
    ...
    log_function_call(logger, "load_entire_ppt_direct", ppt_path=ppt_path, page_indices=page_indices)
    ...
    try:
        ...
        for page_index in page_indices:
            ...
            logger.info(
                f"第 {page_index + 1} 页包含 {page_boxes} 个文本框，{page_paragraphs} 个段落，{total_fragments_in_page} 个文本片段"
            )
            ...
            if page_data["text_boxes"]:
                ...
                for text_box in page_data["text_boxes"]:
                    ...
                    logger.debug(
                        f"{text_box['box_id']:<10} | {text_box['total_paragraphs']:<6} | {total_fragments_in_box:<6} | {preview}"
                    )
        ...
        logger.info(
            f"处理完成：共 {len(pages_data)} 页，{total_boxes} 个文本框，{total_paragraphs} 个段落，{total_fragments} 个文本片段"
        )
...
def main():
    ...
    log_file = os.path.join(logs_dir, "load_ppt_functions_subprocess.log")
    logger = setup_subprocess_logging(log_file)

    logger.info("=" * 60)
    logger.info("启动load_ppt_functions子进程（包含段落层级）")
    logger.info("=" * 60)
    ...
    # 获取项目根目录（load_ppt_functions.py 往上退三层）
    ...
    # 调用直接函数
    result = load_entire_ppt_direct(ppt_path, page_indices)

    if not result:
        logger.error("PPT加载失败")
        return

    logger.info(f"PPT加载完成！总共处理 {len(result.get('pages', []))} 页")

    # 计算统计信息
    stats = result.get("statistics", {})
    logger.info(
        f"统计信息：{stats.get('total_pages', 0)} 页，{stats.get('total_boxes', 0)} 个文本框，{stats.get('total_paragraphs', 0)} 个段落，{stats.get('total_fragments', 0)} 个文本片段"
    )
    ...
    logger.info("load_ppt_functions子进程执行完成")
