    ...
    if model == "qwen":
        ...
        response = client.chat.completions.create(
            ...
            messages=[
                {
                    "role": "system",
                    "content": f"""您是翻译{field}领域文本的专家。接下来，您将获得一系列{source_language}文本（包括短语、句子和单词），他们是隶属于同一个PPT的同一页面下的文本框段落的所有文本。
                                                  请将每一段文本翻译成专业的{target_language}文本。
                                                  1. 上传的将是一个格式化文本，结构如下：
                                                    第1页内容：
...
def separate_translate_text(text_translate):
    ...
    for i, item in enumerate(data):
        ...
        if box_index is not None and paragraph_index is not None:
            ...
            # 将翻译文本按[block]分割成片段
            fragments = [seg.strip() for seg in re.split(r"\[(?:block|块)\]", target_language) if seg.strip()]
...
def validate_page_indices(text_boxes_data):
    ...
    if len(page_indices_sorted) > 1 and page_indices_sorted == list(range(len(page_indices_sorted))):
        ...
        logger.warning(
            "⚠️  如果用户选择的不是连续页面，请检查 ppt_data_utils.py 中的 extract_texts_for_translation 函数"
        )
    else:
