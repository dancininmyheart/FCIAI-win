...
def translate_pdf():
    ...
    try:
        ...
        task_work_dir = os.path.join(pdf_output_dir, f"{os.path.splitext(unique_filename)[0]}_work")
        ...
        try:
            ...
            if zip_url.startswith("file://"):
                ...
                if response.status_code != 200:
                    ...
                    return (
                        jsonify({"success": False, "error": f"下载结果文件失败，状态码: {response.status_code}"}),
                        500,
                    )
        ...
        if not md_file:
            ...
            if enable_image_ocr:
                try:
                    ...
                    ocr_results = process_markdown_images_ocr_and_translate(
                        content, os.path.dirname(md_file), target_language="zh", source_language="en"
                    )
                    ...
                    if ocr_results:
                        ...
                        for i, ocr_result in enumerate(ocr_results):
                            if ocr_result.get("success"):
                                ...
                                logger.info(
                                    f"OCR结果 {i+1}: 图片={os.path.basename(image_path)}, "
                                ...
                                if image_filename in image_name_mapping:
                                    ...
                                    if ocr_text or translation_text:
                                        ...
                                        content = content.replace(
                                            original_image_marker, original_image_marker + ocr_insertion
                                        )
                                ...
                                if translated_dict and cleaned_line in translated_dict:
                                    ...
                                    if translated_dict:
                                                ...
                                                for sentence in sentences:
                                                    ...
                                                    if sentence_stripped:
                                                        for key in translated_dict.keys():
                                                            # 精确匹配或包含关系
                                                            if (
                                                                key.strip() == sentence_stripped
                                                                or sentence_stripped in key.strip()
                                                                or key.strip() in sentence_stripped
                                                            ):
                                        ...
                                        if not matched:
                                            for key in translated_dict.keys():
                                                if (
                                                    cleaned_line.strip() in key.strip()
                                                    or key.strip() in cleaned_line.strip()
                                                ):
                                                    translated_text = translated_dict[key]
                                                    matched = True
                                                    logger.info(
                                                        f"通过模糊匹配找到翻译结果: {line[:30]}... -> {translated_text[:30]}..."
                                                    )
                                                    break
                                        ...
                                        if not matched:
                                            ...
                                            if best_match_key:
                                                ...
                                                matched = True
                                                logger.info(
                                                    f"通过部分匹配找到翻译结果: {line[:30]}... -> {translated_text[:30]}... (相似度: {best_match_score:.2f})"
                                                )
                                        ...
                                        if (
                                            ...
                                            and not translated_text.startswith("[翻译错误:")
                                        ):
                                            processed_lines.append("【译文】" + translated_text)
            ...
            try:
                ...
                # 将语言代码映射为内部使用的代码
                lang_mapping = {"en": "en", "zh": "zh", "ja": "ja", "english": "en", "chinese": "zh", "japanese": "ja"}
            ...
            if not conversion_success:
                try:
                    ...
                    for i, paragraph in enumerate(final_paragraphs):
                        ...
                        # 精确匹配
                        if (
                            paragraph in translation_lookup
                            and translation_lookup[paragraph].strip()
                            and not translation_lookup[paragraph].startswith("[翻译错误:")
                        ):
                        ...
                        if not translated_text:
                            for orig_text, trans_text in list(translation_lookup.items()):
                                ...
                                if (
                                    trans_text.strip()
                                    and not trans_text.startswith("[翻译错误:")
                                    and (
                                        orig_text.startswith(paragraph)
                                        or paragraph.startswith(orig_text)
                                        or len(set(orig_text.split()) & set(paragraph.split()))
                                        / max(len(orig_text.split()), 1)
                                        > 0.7
                                    )
                                ):
                        ...
                        if not translated_text and translated_dict:
                            ...
                            for orig_text, trans_text in list(translated_dict.items()):
                                if (
                                    trans_text.strip()
                                    and not trans_text.startswith("[翻译错误:")
                                    and orig_text in translation_lookup
                                ):
                                    translated_text = trans_text
                                    del translation_lookup[orig_text]
                                    break
                        ...
                        # 这样可以避免在最终结果中显示"翻译失败"或"[未找到匹配译文]"
                            ...
                            try:
                                ...
                                # 调用翻译函数
                                retry_result = translate_qwen(
                                    paragraph, field, stop_words, custom_translations, "auto", "zh"
                                )
                                    ...
                                    if next_line.startswith("[OCR识别结果]:"):
                                        ...
                                        while ocr_end < len(lines):
                                            if lines[ocr_end].strip().startswith("[OCR翻译结果]:") or (
                                                ocr_end + 1 < len(lines)
                                                and lines[ocr_end + 1].strip().startswith("![")
                                                and "](" in lines[ocr_end + 1]
                                                and lines[ocr_end + 1].strip().endswith(")")
                                            ):
                                                break
                                            ocr_end += 1
                                        ...
                                        if ocr_end < len(lines) and lines[ocr_end].strip().startswith("[OCR翻译结果]:"):
                                            ...
                                            while trans_end < len(lines):
                                                if (
                                                    trans_end + 1 < len(lines)
                                                    and lines[trans_end + 1].strip().startswith("![")
                                                    and "](" in lines[trans_end + 1]
                                                    and lines[trans_end + 1].strip().endswith(")")
                                                ):
                                                    break
                                                trans_end += 1
        ...
        try:
            ...
            logger.info(
                f"UploadRecord属性: filename={record.filename}, stored_filename={record.stored_filename}, file_path={record.file_path}, user_id={record.user_id}, file_size={record.file_size}, status={record.status}"
            )
            logger.info(f"准备添加记录到会话")
            db.session.add(record)
            logger.info(f"记录已添加到会话")
            logger.info(
                f"准备提交记录: filename={record.filename}, stored_filename={record.stored_filename}, file_path={record.file_path}, user_id={record.user_id}, file_size={record.file_size}, status={record.status}"
            )
        ...
        response = jsonify(
            {
                ...
                "download_url": url_for(
                    "main.download_translated_pdf", filename=os.path.basename(docx_path), _external=True
                ),
            }
        )
...
def delete_pdf_translation_by_id(record_id):
    ...
    try:
        # 查询记录，确保是PDF翻译记录
        record = UploadRecord.query.filter_by(id=record_id, user_id=current_user.id, status="completed").first()
...
def batch_upload_translations():
    ...
    try:
        ...
        if file_ext not in EXCEL_ALLOWED_EXTENSIONS:
            return (
                jsonify({"error": f'不支持的文件格式: .{file_ext}。只支持: {", ".join(EXCEL_ALLOWED_EXTENSIONS)}'}),
                400,
            )
